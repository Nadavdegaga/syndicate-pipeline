// Affise daily-stats ingest endpoint.
...
function normalize(r: IngestRow): { ok: true; row: Record<string, unknown> } | { ok: false; error: string } {
  ...
  return {
    ...
    row: {
      ...
      source: r.source && String(r.source).trim() ? String(r.source).trim() : "affise",
      clicks,
      conversions,
      revenue,
      cost,
...
export async function POST(request: Request) {
  ...
  if (!scopes.includes("affise") && !scopes.includes("*")) {
    return NextResponse.json(
      { error: "Key does not have 'affise' scope" },
      { status: 403 },
    );
  }
  ...
  // Upsert (PostgREST upsert doesn't easily distinguish inserted vs updated;
  // we surface total processed.)
  ...
  await supabase.from("activity_log").insert({
    entity_type: "offer", // closest existing entity_type
    entity_id: "00000000-0000-0000-0000-000000000000",
    action: "affise_ingest",
