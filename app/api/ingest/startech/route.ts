// StarTech daily-stats ingest endpoint.
...
function normalize(r: IngestRow): { ok: true; row: Record<string, unknown> } | { ok: false; error: string } {
  ...
  return {
    ...
    row: {
      ...
      source: r.source && String(r.source).trim() ? String(r.source).trim() : "startech",
      clicks,
      conversions,
      revenue,
      cost,
...
export async function POST(request: Request) {
  ...
  if (!scopes.includes("startech") && !scopes.includes("*")) {
    return NextResponse.json(
      { error: "Key does not have 'startech' scope" },
      { status: 403 },
    );
  }
