// StarTech platform client — runs on Affise's API.
...
async function callOffersPage(
  ...
  if (!base) throw new Error("StarTech base URL is required (e.g. https://api.<your-startech>.com/3.0)");
  ...
  if (!r.ok) {
    ...
    throw new Error(`StarTech HTTP ${r.status}: ${text.slice(0, 200)}`);
  }
