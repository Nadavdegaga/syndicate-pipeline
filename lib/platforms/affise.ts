// Affise REST API client.
// Docs: https://affise.atlassian.net/wiki/spaces/Affise/pages/2785706/3.0+API
...
async function callOffersPage(
  ...
  if (!base) throw new Error("Affise base URL is required (e.g. https://api.<your>.com/3.0)");
  ...
  if (!r.ok) {
    ...
    throw new Error(`Affise HTTP ${r.status}: ${text.slice(0, 200)}`);
  }
  const json = await r.json();
  // Affise payload: { status: 1, offers: [...], pagination: { ... } }
