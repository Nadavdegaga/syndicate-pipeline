// Fetches daily performance stats from Nomi's Affise instance — GET /3.0/stats/getbyprogram.
// Call with a single date (date_from = date_to) to get per-offer stats for that day.
// Env vars required: NOMI_API_KEY, NOMI_BASE_URL (e.g. https://api.nomi-domain.com)
...
export async function fetchNomiStatsForDate(
  date: string,
): Promise<{ ok: true; rows: NormalizedStatRow[] } | { ok: false; error: string }> {
  const apiKey = process.env.NOMI_AFFISE_API_KEY;
  const base = (process.env.NOMI_AFFISE_BASE_URL ?? "").replace(/\/+$/, "");

  if (!apiKey) return { ok: false, error: "NOMI_AFFISE_API_KEY env var not set" };
  if (!base) return { ok: false, error: "NOMI_AFFISE_BASE_URL env var not set" };
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return { ok: false, error: `Nomi HTTP ${res.status}: ${text.slice(0, 300)}` };
      }
      json = await res.json();
    } catch (e) {
