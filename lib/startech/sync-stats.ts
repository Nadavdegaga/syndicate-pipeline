// Syncs StarTech stats for a single date into startech_daily_stats via service client.

import { createServiceClient } from "@/lib/supabase/service";
import { fetchStarTechStatsForDate } from "./stats-client";
...
export async function syncStarTechStatsForDate(date: string): Promise<StatsSyncResult> {
  const fetched = await fetchStarTechStatsForDate(date);
  if (!fetched.ok) {
