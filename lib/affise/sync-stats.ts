// Syncs Affise stats for a single date into affise_daily_stats via service client.

import { createServiceClient } from "@/lib/supabase/service";
import { fetchAffiseStatsForDate } from "./stats-client";
...
export async function syncAffiseStatsForDate(date: string): Promise<StatsSyncResult> {
  const fetched = await fetchAffiseStatsForDate(date);
  if (!fetched.ok) {
