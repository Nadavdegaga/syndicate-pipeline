...
export function AffiseDashboard({ rows }: { rows: DailyStat[] }) {
  ...
  // Daily series (group by report_date)
  ...
  if (rows.length === 0) {
    return <EmptyAffiseState />;
  }

  return (
    <div className="space-y-6" data-tour="affise-dashboard">
      {/* Date range picker */}
      <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
        ...
        {(
          ...
          <button
            key={k}
            onClick={() => setRange(k)}
            data-tour={k === "7d" ? "affise-range" : undefined}
            className={cn(
      ...
      {/* KPIs */}
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4" data-tour="affise-kpis">
        <KpiWithDelta
          label="Clicks"
          value={cur.clicks}
          prev={prev.clicks}
...
function EmptyAffiseState() {
  return (
    <div
      className="rounded-2xl border border-dashed border-slate-200 bg-white p-8"
      data-tour="affise-empty-state"
    >
      <div className="mx-auto max-w-xl space-y-4 text-center">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Once your Affise stats are flowing in, you&apos;ll see KPIs,
            daily trends, and a per-offer breakdown here.
          </p>
        </div>
        <Separator />
        <div className="space-y-2 text-left text-sm">
          <p className="font-medium text-slate-900">
            How to send data (for your engineer)
          </p>
          <ol className="list-decimal space-y-2 pl-5 text-slate-700">
            ...
            <li>
              ...
              (YYYY-MM-DD) and{" "}
              <code className="rounded bg-slate-100 px-1 text-xs">offer_id</code>
              . Numeric:{" "}
              ...
              <code className="rounded bg-slate-100 px-1 text-xs">cost</code>
              . Re-sends for the same{" "}
              <code className="rounded bg-slate-100 px-1 text-xs">
                (report_date, offer_id, source)
              </code>{" "}
              are idempotent.
            </li>
          </ol>
