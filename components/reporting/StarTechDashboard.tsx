...
export function StarTechDashboard({
  rows,
  canSync = false,
}: {
  rows: DailyStat[];
  ...
  async function handleSync() {
    ...
    setSyncMsg("");
    try {
      const res = await fetch("/api/startech/sync", {
        method: "POST",
  ...
  if (rows.length === 0) {
    return <EmptyStarTechState canSync={canSync} syncDate={syncDate} setSyncDate={setSyncDate} onSync={handleSync} syncState={syncState} syncMsg={syncMsg} />;
  }

  return (
    <div className="space-y-6" data-tour="startech-dashboard">
      {/* Sync bar */}
      {canSync && (
        <div className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
          <RefreshCw className={cn("h-4 w-4 text-slate-400", syncState === "syncing" && "animate-spin")} />
          <span className="text-xs font-medium text-slate-600">Sync from StarTech</span>
          <input
            type="date"
            value={syncDate}
          ...
          <span className="ml-auto text-xs text-slate-400">Runs automatically every day at 04:00 UTC</span>
        </div>
      )}
  ...
  return (
    <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-8">
      <div className="mx-auto max-w-xl space-y-4 text-center">
        ...
        <div>
          <h2 className="text-lg font-semibold text-slate-900">No StarTech data yet</h2>
          <p className="mt-1 text-sm text-slate-500">
            Once your StarTech stats are flowing in, you&apos;ll see KPIs, daily trends, and a
        ...
        {canSync ? (
          <div className="space-y-3 text-left text-sm">
            <p className="font-medium text-slate-900">Pull stats directly from StarTech</p>
            <p className="text-slate-500">
              Your StarTech credentials are configured. Pick a date and sync to get started.
            </p>
          ...
          <div className="space-y-2 text-left text-sm">
            ...
            <ol className="list-decimal space-y-2 pl-5 text-slate-700">
              ...
              <li>
                POST daily stats to{" "}
                <code className="rounded bg-slate-100 px-1 text-xs">/api/ingest/startech</code>{" "}
                with header{" "}
